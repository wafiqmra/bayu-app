                ...
                <div class="lg:col-span-5 space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div class="bg-white p-4 rounded-xl shadow-sm border border-[#D4E0E8]">
                            ...
                            <h3 class="text-md font-bold text-[#1E3A5F]">{{ $stats['peminjam_aktif'] ?? 0 }} Orang</h3>
                        </div>
                    </div>
                        ...
                        <form action="/tambah-utang" method="POST" class="space-y-4">
                            ...
                            <div class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <input type="tel" name="nomor_wa" x-model="wa" class="w-full border border-[#D4E0E8] rounded-lg p-2.5 focus:border-[#1E3A5F] focus:ring-1 focus:ring-[#1E3A5F]/10 outline-none transition bg-white text-sm" placeholder="08xxxxxxxxxx">
                                </div>
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-[10px] font-medium text-[#5F7D9C] mb-1 uppercase tracking-wider">Jumlah (Rp)</label>
                                        <input type="number" name="jumlah_utang" required class="w-full border border-[#D4E0E8] rounded-lg p-2.5 focus:border-[#1E3A5F] focus:ring-1 focus:ring-[#1E3A5F]/10 outline-none transition bg-white text-sm" placeholder="50000">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] font-medium text-[#5F7D9C] mb-1 uppercase tracking-wider">Janji Bayar</label>
                                        <input type="date" name="jatuh_tempo" class="w-full border border-[#D4E0E8] rounded-lg p-2.5 focus:border-[#1E3A5F] focus:ring-1 focus:ring-[#1E3A5F]/10 outline-none transition bg-white text-sm">
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-[10px] font-medium text-[#5F7D9C] mb-1 uppercase tracking-wider">Keterangan</label>
                                    <input type="text" name="keterangan" class="w-full border border-[#D4E0E8] rounded-lg p-2.5 focus:border-[#1E3A5F] focus:ring-1 focus:ring-[#1E3A5F]/10 outline-none transition bg-white text-sm" placeholder="Buat apa?">
                                </div>
                            </div>
                            <button type="submit" class="w-full bg-[#1E3A5F] hover:bg-[#0F2A47] text-white py-3 rounded-lg font-bold text-sm transition-all shadow-sm active:scale-[0.98] mt-2 uppercase tracking-widest text-center">
                                💰 Simpan Catatan
                            </button>
                        </form>
                    </div>
                                ...
                                <details class="group border border-[#D4E0E8] rounded-xl overflow-hidden shadow-sm">
                                    ...
                                    <div class="p-4 bg-white space-y-3">
                                        @foreach($items as $debt)
                                            ...
                                            @endphp
                                            <div class="p-3 rounded-lg border {{ $isOverdue ? 'border-red-200 bg-red-50/30' : 'border-slate-100' }} flex items-center justify-between">
                                                <div class="flex-1">
                                                    <p class="text-xs font-bold text-[#1E3A5F]">{{ $debt->keterangan ?: 'Tanpa Keterangan' }} @if($isOverdue) <span class="text-[8px] bg-red-500 text-white px-1.5 py-0.5 rounded-full ml-1 font-black">TELAT!</span> @endif</p>
                                                ...
                                                <div class="flex gap-2">
                                                    @if($wa)
                                                        <a href="https://wa.me/{{ $wa }}?text={{ urlencode($msgWA) }}" target="_blank" class="p-2 rounded-lg border border-emerald-100 hover:bg-emerald-50"><img src="{{ asset('whatsapp.png') }}" class="w-4 h-4"></a>
                                                    @endif
                                    ...
                                    <div class="p-4 bg-white space-y-3">
                                        @foreach($items as $debt)
                                            <div class="p-3 rounded-lg border border-slate-100 flex items-center justify-between">
                                                ...
                                                <form action="/update-status/{{ $debt->id }}" method="POST">@csrf <button class="text-[10px] bg-[#1E3A5F] text-white px-4 py-1.5 rounded-full font-black uppercase shadow-sm">Batal</button></form>
                                            </div>
                                        @endforeach
                                    </div>
                                </details>
